...
export default function ItineraryForm({
    ...
    const [packageEnabled, setPackageEnabled] =
        useState(
            (initialItinerary?.packageOptions?.length ??
                0) > 0
        );

    const [hotelEnabled, setHotelEnabled] =
        useState(
            initialItinerary?.hotelEnabled ?? false
        );
    ...
    const addHotel = () => {
        setHotels((current) => {
            const previousHotel = current[current.length - 1];
            ...
            return [
                ...current,
                {
                    ...
                    // New hotel's check-in = previous hotel's check-out
                    checkIn: previousHotel?.checkOut ?? "",

                    // User will enter the new hotel's check-out
                    checkOut: "",
